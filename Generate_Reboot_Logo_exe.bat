...
:: 安装依赖
REM ::设置目标文件夹路径
set DEST_FOLDER_DIST=dist\logo
set DEST_FOLDER_BUILD=build\logo
set DEST_FOLDER_INTERNAL=_internal
set FILE_NAME=logo.spec
set EXE_NAME=logo.exe
...
REM ::打包exe文件 --noconsole  --uac-admin
::pyinstaller -F -w main.py --hidden-import=pkg_resources.py2_warn --hidden-import=pkg_resources.extern
poetry run pyinstaller --noconsole %PYTHON_SCRIPT%

REM
echo copy file operation！！！
::拷贝文件夹，文件到当前目录下
set SOURCE_DICT_EXE=dist\logo\logo.exe
set SOURCE_DICT_INTERNAL=dist\logo\_internal
